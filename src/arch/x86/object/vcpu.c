...
#ifdef CONFIG_ARCH_X86_64
void
vcpu_restore_guest_msrs(vcpu_t *vcpu)
{
...
#ifdef CONFIG_ARCH_X86_64
static exception_t
invokeWriteMSR(vcpu_t *vcpu, word_t *buffer, word_t field, word_t value)
{
    tcb_t *thread;
...
static exception_t
decodeVCPUWriteMSR(cap_t cap, word_t length, word_t *buffer)
{
    word_t field;
    word_t value;
...
static exception_t
invokeReadMSR(vcpu_t *vcpu, word_t field, word_t *buffer)
{
    tcb_t *thread;
    thread = NODE_STATE(ksCurThread);
...
static exception_t
decodeVCPUReadMSR(cap_t cap, word_t length, word_t *buffer)
{
    if (length < 1) {
